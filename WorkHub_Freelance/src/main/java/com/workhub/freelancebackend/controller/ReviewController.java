...
//    }
    ...
    public ResponseEntity<List<Review>> getAllReviews() {
        ...
        List<Review> dtos = reviews.stream().map(r -> {
            String freelancerName = userRepository.findById(r.getFreelancerId()).map(u -> u.getName()).orElse("Unknown");
            String clientName = userRepository.findById(r.getClientId()).map(u -> u.getName()).orElse("Unknown");
            return new Review(r.getId(), r.getFreelancerId(), freelancerName, r.getClientId(), clientName, r.getRating(), r.getFeedback());
        }).toList();
